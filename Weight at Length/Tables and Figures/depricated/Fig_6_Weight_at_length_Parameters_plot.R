...
plot(NA,NA, main = NA, xlab =param.names[1], yaxt = "n", ylab= NA, xlim = c(0.000015, 0.000075), ylim = c(-1.5,6.5), cex.lab = 1.25)
sex <- c("Female","Male")
...
for ( j in c(1:8)){
  for ( i in 1:2){
    ...
    dat.sub <- exp(as.matrix(betas_a_re) %*% as.vector(c(1, i, lat_lon_df[which(lat_lon_df$state_no_check == state_ind),2])) + log_a_re)
...
plot(NA,NA, main = NA, xlab =param.names[2], yaxt = "n", ylab= NA, xlim = c(2.85, 3.1), ylim = c(-1.5,6.5), cex.lab = 1.25)
sex <- c("Female","Male")
