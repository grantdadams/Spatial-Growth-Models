...
state.id <- data.frame(state = c("VA Ocean","VA Bay","NC","SC","FL Atlantic","FL Gulf","AL","MS"), state_no_check = c(1:length(unique(dat$state))))
...
########################################################
figure_names <- c("Tables and Figures/Fig_A7_Paired_a_Parameter_Differences_wrong.tiff", 
                  "Tables and Figures/Fig_A8_Paired_b_Parameter_Differences_wrong.tiff")
...
xlim <- list(c(-.000015, .000028),
             c(-0.1, 0.08))

xaxis_tick <- list(seq(-.00002, .00002, by = 0.00001), 
                   seq(-0.1, .2, by = .05))

xaxis_lab <- list(c(NA, NA,0, NA, 2e-5),
                  c(-0.1, NA, 0, NA, NA, NA, NA))
