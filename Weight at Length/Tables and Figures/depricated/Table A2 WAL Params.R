...
for(i in c(state_no_df$state_no_check)){
  ...
  F_a <- exp(as.matrix(betas_a_re) %*% as.vector(c(1, 0, lat_lon_df[which(lat_lon_df$state_no_check == i),2])) + log_a_re)
  M_a <- exp(as.matrix(betas_a_re) %*% as.vector(c(1, 1, lat_lon_df[which(lat_lon_df$state_no_check == i),2])) + log_a_re)
...
for(i in c(state_no_df$state_no_check)){
  ...
  F_b <- exp(as.matrix(betas_b_re) %*% as.vector(c(1, 0, lat_lon_df[which(lat_lon_df$state_no_check == i),2])) + log_b_re)
  M_b <- exp(as.matrix(betas_b_re) %*% as.vector(c(1, 1, lat_lon_df[which(lat_lon_df$state_no_check == i),2])) + log_b_re)
