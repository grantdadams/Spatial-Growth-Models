...
parameters_to_save <- c(paste("Betas_log_a[",1:3,"]", sep = ""), paste("Betas_log_b[",1:3,"]", sep = ""), "sigma_a", "sigma_b", "sigma", paste("log_a_re[",c(1:9), "]", sep = ""), paste("log_b_re[",c(1:9), "]", sep = ""))
...
dataList = list(weight = dat$wet_weight_grams, region = dat$state_no_check, length = dat$FL_mm, n = nrow(dat), Jtrunc = 8, J = 9, Pred = Predictors, n_pred = ncol(Predictors), state = factor(dat$state, levels = c("TX", "LA", "MS", "AL", "FL_gulf", "FL_atlantic", "SC", "NC", "VA_ocean", "VA_bay")))
...
for(i in 1:nrow(draws)){
  ...
  a.sub <- (as.matrix(dataList$Pred) %*% as.vector(betas_a_re) + log_a_re)
  ...
  pred.weight <- a.sub + log(dataList$length) * b.sub 
  resid <- log(dataList$weight) - pred.weight
  sum_sq <- resid^2
  
  #### PREDICTED BIT ####
  # generate new data and comput fit
  sigma <- as.numeric(draws[i,"sigma"])
  weight.new <- rnorm(n = length(pred.weight), mean = (pred.weight), sd = sigma)
