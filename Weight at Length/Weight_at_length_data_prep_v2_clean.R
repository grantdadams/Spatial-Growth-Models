...
load("Sheepshead_data_GOM.RData" )

# Female is 1, male is 2
dat$sex <- ifelse(dat$sex == "F", 1, ifelse(dat$sex == "M", 2, NA))
...
state.id <- data.frame(state = c("VA Ocean","VA Bay","NC","SC","FL Atlantic","FL Gulf","AL","MS","LA","TX"), state_no = c(1:10))

Check.state.df <- data.frame(state = c(unique(dat$state)), state_no_check = c(1:length(unique(dat$state))))


dat <- merge(dat, Check.state.df, by = "state")

Check.state.df <- data.frame(state = c("AL", "FL Atlantic","FL Gulf","MS", "NC", "SC", "VA Bay", "VA Ocean"), state.unique = unique(dat$state), state_no_check = c(1:length(unique(dat$state))))
state.id <- merge(Check.state.df, state.id, by = "state")
...
pred <- data.frame(predict(mod_simp, data.frame(log(dat$FL_mm)), interval = "prediction", level=.999999))
...
for ( j in 1:length(unique(dat$state_no_check))){
  dat.sub <- subset(dat, dat$state_no_check == j)
  if (j == 1){
...
legend("topleft", legend = c(unique(dat$state_no_check)), col = c(1:8), pch = 16, cex = 1.25, bty ="n", inset = -0.02)
