...
tiff(file="Tables and Figures/Fig_6_Weight_at_length_parameters.tiff" , height= 77.27, pointsize=12,  width=170 , res=300  , units = "mm", family = "serif")
...
plot(NA,NA, main = NA, xlab =param.names[1], yaxt = "n", ylab= NA, xlim = c(0.000024, 0.000064), ylim = c(-1.5,6.5), cex.lab = 1.25)
sex <- c("Female","Male")
...
legend("bottomright", c(sex), pch = c(NA,NA), lty = c(1,1), lwd=c(3,3), col = c(cols), cex = 0.8, bty ="n")
legend("topleft", c(paste("(", letters, ")", sep = ""))[1], cex = 1, bty ="n", adj = 1)



plot(NA,NA, main = NA, xlab =param.names[2], yaxt = "n", ylab= NA, xlim = c(2.86, 3), ylim = c(-1.5,6.5), cex.lab = 1.25)
sex <- c("Female","Male")
...
legend("topleft", c(paste("(", letters, ")", sep = ""))[2], cex = 1, bty ="n", adj = 1)
dev.off()
