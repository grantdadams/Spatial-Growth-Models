...
tiff(file="Tables and Figures/Fig_2_CAR_VBGF midway Plots 8095.tiff" , height= 77.27, pointsize=12,  width=170 , res=300  , units = "mm", family = "serif")
...
#plot K density
plot(NA,NA, main = NA, xlab =expression(italic("k"["s,r"])~(yr^-1)), yaxt = "n", ylab= NA, xlim = c(.15, 0.6), ylim = c(-3.5,6.5), cex.lab = 1.25)
sex <- c("Female","Male")
for ( j in 1:length(unique(dat$state))){
  for ( i in 1:2){
    state_ind <- c(1:4,6:11)[j] # Indexing variable
