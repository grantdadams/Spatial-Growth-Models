...
state.id <- data.frame(state = c("VA Ocean","VA Bay","NC","SC","FL Atlantic","FL Gulf","AL","MS","LA","TX"), state_no = c(1:4, 6:11))
library(FSA)

cols <- c("#007FFF","#FF7F00") # Colors for the VBGF lines
cols.points <- c("#80bfff", "#ffc180")

tiff(file="Histogram_of_Lengths.tiff" , height= 10, pointsize=18,  width=7 , res=300  , units = "in", family = "serif")
par(mar=c(0.5 , 1.75 , .25 , .5) +0.1, tcl=-.25 , mgp=c(2.5,  .7 ,  0) ,  oma=c(0 , 0, 0 , 0), cex = 0.75)
layout(matrix(c(1:18), 6, 3, byrow = T ))

loop.ind <- c(NA,1,2,NA,3,4,NA,6,7,NA,8,9,NA,10,11,NA,NA,NA)    
for ( j in 1:18){
  if(j %in% c(1,4,7,10,13,16,17,18)){plot.new()}
  else{
    
    dat.sub.females <- dat[which(dat$state_no == loop.ind[j] & dat$sex == 0),]
    dat.sub.males <- dat[which(dat$state_no == loop.ind[j] & dat$sex == 1),]
...
tiff(file="Histogram_of_ages.tiff" , height= 121.4, pointsize=11,  width=85 , res=300  , units = "mm", family = "serif")
...
loop.ind <- c(NA,1,2,NA,3,4,NA,6,7,NA,8,9,NA,10,11,NA,NA,NA)
loop.ind2 <- c(NA,1,2,NA,3,4,NA,5,6,NA,7,8,NA,9,10,NA,NA,NA) 
for ( j in 1:18){
  if(is.na(loop.ind[j]) == T){plot.new()}
    ...
    if(loop.ind[j] %in% c(1)){
      legend("topright", c(paste("(", letters, ") ", state.id[which(state.id$state_no == loop.ind2[j]),1], sep = ""))[loop.ind2[j]], bty = "n", cex = 1)
      legend(x = 20, y = 25, c("Females", "Males"), lwd = c(2,2), bty = "n", col = c(cols), cex = 0.75)
    }
    if(loop.ind[j] %in% c(2:11)){
      legend("topright", c(paste("(", letters, ") ", state.id[which(state.id$state_no == loop.ind[j]),1], sep = ""))[loop.ind2[j]], bty = "n", cex = 1)
    }
  ...
  if (loop.ind[j] %in% c(10, 11)){
    axis(side = 1)
    mtext(side = 1, "Fractional Age (yr)", line = 2,cex=0.75)}
  if (loop.ind2[j] %in% c(5)){
    mtext(side = 2, "Count", line = 2.3, cex = 1)
  }
}
